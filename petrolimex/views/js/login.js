...
$(document).ready(function() {
    ...
    $(document).on('click', "#login",function() {
        ...
        axios.post('/login',obj).then(res => {
            if(Object.keys(res.data).indexOf('user') !== -1) {
                $('div.login').remove();
                localStorage.setItem('user', JSON.stringify(res.data.user));
            } else {
                $('div.login table').before('<tr><td colspan="2" style="color:red;background-color:yellow;"><h4>Tài khoản không tồn tại hoặc username, password nhập sai.</h4></td></tr>');
            }
